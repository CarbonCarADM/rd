...
function NewsPage({ onBack }: { onBack: () => void }) {
  ...
  const GNEWS_API_KEY = '8bcd801adc73430c7ae1e3bee3733839';

  const SOURCES = [
    { label: 'Música',   q: 'música' },
    { label: 'Artistas', q: 'artista cantor banda' },
    { label: 'Shows',    q: 'show festival concerto' },
  ];

  const MOCK_NEWS: NewsItem[] = [
    { title: 'Beyoncé anuncia nova turnê mundial para 2026', description: 'A cantora confirmou datas para América do Sul, incluindo três shows no Brasil em março de 2026.', link: '#', pubDate: '11 jun', thumbnail: 'https://picsum.photos/seed/music1/400/225', author: 'Rolling Stone BR' },
    { title: 'Festival Lollapalooza divulga line-up completo', description: 'Edição 2026 traz headliners internacionais e nomes do indie nacional no Autódromo de Interlagos.', link: '#', pubDate: '10 jun', thumbnail: 'https://picsum.photos/seed/music2/400/225', author: 'Billboard BR' },
    { title: 'Novo álbum de Ludmilla quebra recordes no Spotify', description: 'Projeto foi o mais ouvido em 24 horas na plataforma, com mais de 12 milhões de streams.', link: '#', pubDate: '10 jun', thumbnail: 'https://picsum.photos/seed/music3/400/225', author: 'Vagalume' },
    { title: 'The Weeknd confirma show em São Paulo', description: 'Cantor canadense retorna ao Brasil após cinco anos com a turnê After Hours Til Dawn.', link: '#', pubDate: '09 jun', thumbnail: 'https://picsum.photos/seed/music4/400/225', author: 'G1 Música' },
    { title: 'IVE e BLACKPINK lideram charts do K-Pop no Brasil', description: 'Grupos sul-coreanos dominam as paradas nacionais com lançamentos da última semana.', link: '#', pubDate: '09 jun', thumbnail: 'https://picsum.photos/seed/music5/400/225', author: 'POPline' },
    { title: 'Gusttavo Lima bate recorde de público em Goiânia', description: 'Show reuniu mais de 80 mil pessoas e foi o maior evento de sertanejo do ano no país.', link: '#', pubDate: '08 jun', thumbnail: 'https://picsum.photos/seed/music6/400/225', author: 'Terra Música' },
    { title: 'Grammy Latino 2026 anuncia indicados', description: 'Anitta, Ivete Sangalo e Gilberto Gil estão entre os artistas brasileiros na disputa.', link: '#', pubDate: '08 jun', thumbnail: 'https://picsum.photos/seed/music7/400/225', author: 'Billboard BR' },
    { title: 'Raul Seixas completa 50 anos do álbum Novo Aeon', description: 'Relançamento especial com faixas remasterizadas chega às plataformas digitais neste mês.', link: '#', pubDate: '07 jun', thumbnail: 'https://picsum.photos/seed/music8/400/225', author: 'Rolling Stone BR' },
    { title: 'Matuê lança clipe com mais de 5 milhões de views em 24h', description: 'Videoclipe do novo single foi gravado em Tóquio e já figura no trending do YouTube Brasil.', link: '#', pubDate: '07 jun', thumbnail: 'https://picsum.photos/seed/music9/400/225', author: 'Rap Nacional' },
    { title: 'Rock in Rio 2026 esgota ingressos em tempo recorde', description: 'Todos os passaportes do festival foram vendidos em menos de duas horas após abertura das vendas.', link: '#', pubDate: '06 jun', thumbnail: 'https://picsum.photos/seed/music10/400/225', author: 'G1 Música' },
  ];

  const newsCache = React.useRef<Record<number, { items: NewsItem[]; ts: number }>>({});

  const fetchNews = async (sourceIndex: number) => {
    // Usa cache se tiver menos de 60 minutos
    ...
    try {
      const q = encodeURIComponent(SOURCES[sourceIndex].q);
      const url = `https://gnews.io/api/v4/search?q=${q}&lang=pt&country=br&max=10&apikey=${GNEWS_API_KEY}`;
      const res = await fetch(url);
      if (!res.ok) throw new Error('HTTP ' + res.status);
      const data = await res.json();
      if (!data.articles?.length) throw new Error('Sem artigos');
      ...
      newsCache.current[sourceIndex] = { items, ts: Date.now() };
      setNews(items);
    } catch {
      // CORS no AI Studio — usa mock para visualizar o layout
      // Na Vercel isso não acontece e a API real é usada
      ...
      <div className="flex gap-2 px-4 pb-4 shrink-0 overflow-x-auto no-scrollbar">
        {SOURCES.map((s, i) => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-xs font-bold tracking-wider uppercase whitespace-nowrap transition-all active:scale-95 min-h-[36px] border ${
              activeSource === i
                ? 'text-white border-red-600/60'
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center gap-3 py-16">
            <motion.div
              ...
              className="w-8 h-8 rounded-full border-2 border-white/10 border-t-red-500"
            />
        ...
        {error && !loading && (
          <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
            ...
            <div>
              ...
              <p className="text-white/30 text-sm mt-1">Verifique sua conexão</p>
            </div>
            <button
              onClick={() => fetchNews(activeSource)}
              className="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold text-white border border-white/15 bg-white/8 hover:bg-white/12 transition-all"
            >
              <RefreshCw size={14} />
              Tentar novamente
            </button>
          ...
          >
            ...
            {news[0] && (
              <motion.a
                ...
                className="col-span-2 rounded-2xl overflow-hidden bg-white/8 border border-white/10 hover:bg-white/12 transition-all active:scale-[0.99] group"
              >
                {news[0].thumbnail && (
                  <div className="relative w-full aspect-video overflow-hidden">
                    ...
                    <span
                      className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest text-white border border-red-500/50"
                      style={{ background: 'linear-gradient(135deg, #c41e10cc, #8b1208cc)' }}
                    >
                      Destaque
                    </span>
                  </div>
                ...
                <div className="p-4 flex items-start justify-between gap-2">
                  <div className="flex-1">
                    <p className="text-sm font-bold leading-snug line-clamp-2">{news[0].title}</p>
                    <p className="text-white/40 text-xs mt-1.5 line-clamp-2 leading-relaxed">{news[0].description}</p>
                    <div className="flex items-center gap-2 mt-2.5">
                      <span className="text-[9px] font-bold text-white/30 uppercase tracking-widest">{news[0].author}</span>
                      <span className="text-white/20">·</span>
                      <span className="text-[9px] text-white/30">{news[0].pubDate}</span>
                    </div>
                  </div>
                  <ExternalLink size={13} className="text-white/25 shrink-0 mt-0.5" />
                </div>
              </motion.a>
            )}
            ...
            {news.slice(1).map((item, i) => (
              <motion.a
                ...
                className="rounded-2xl overflow-hidden bg-white/8 border border-white/10 hover:bg-white/12 transition-all active:scale-[0.98] group flex flex-col"
              >
                  ...
                  {item.thumbnail ? (
                    <img
                      ...
                      onError={(e) => {
                        ...
                        el.parentElement!.innerHTML = `<div class="w-full h-full flex items-center justify-center"><svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' fill='none' stroke='rgba(255,255,255,0.15)' stroke-width='2'><path d='M9 18V5l12-2v13'/><circle cx='6' cy='18' r='3'/><circle cx='18' cy='16' r='3'/></svg></div>`;
                      }}
                    />
                  ) : (
                ...
                <div className="p-3 flex flex-col flex-1">
                  <p className="text-xs font-bold leading-snug line-clamp-3 flex-1">{item.title}</p>
